...
func Go루틴_소켓_C함수_호출(ch초기화 chan lib.T신호) (에러 error) {
	...
	소켓REP = 확인(lib.New소켓REP_raw(lib.P주소_Xing_C함수_호출)).(mangos.Socket) // 종료할 때 닫음.
...
func c함수_호출_도우미(ch초기화 chan<- lib.T신호) (에러 error) {
	...
	for {
		...
		default:
			...
			질의값 := 확인(수신_메시지.G해석값(0, xt.F바이트_변환값_해석))
			...
			case xt.P함수_질의:
				...
				식별번호, 에러 = func() (식별번호 int, 에러 error) {
					defer lib.S에러패닉_처리기{M에러_포인터:&에러}.S실행()
					TR질의값 := 확인(질의값.(*xt.S호출_인수_질의).M질의값.G해석값(xt.F바이트_변환값_해석))
					return f조회_및_주문_질의_처리(TR질의값.(lib.I질의값))
				}()
			...
			case xt.P함수_에러_메시지:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F에러_메시지(질의값.(xt.S호출_인수_정수값).M정수값))
			case xt.P함수_TR쿼터:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F초당_TR쿼터(질의값.(xt.S호출_인수_문자열).M문자열))
			case xt.P함수_계좌_수량:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_수량())
			case xt.P함수_계좌_번호:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_번호(질의값.(xt.S호출_인수_정수값).M정수값))
			case xt.P함수_계좌_이름:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_이름(질의값.(xt.S호출_인수_문자열).M문자열))
			case xt.P함수_계좌_상세명:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_상세명(질의값.(xt.S호출_인수_문자열).M문자열))
			case xt.P함수_압축_해제:
...
func f소켓_질의_회신(회신_메시지 lib.I소켓_메시지, raw메시지 *mangos.Message, ch종료 chan lib.T신호) {
	...
	default:
		확인(회신_메시지.S소켓_회신(소켓REP, lib.P30초, raw메시지))
