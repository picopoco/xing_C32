...
func c함수_호출_도우미(ch초기화 chan<- lib.T신호) (에러 error) {
	...
	for {
		...
		default:
			...
			호출_인수_후보 := 수신_메시지.S해석기(xt.F바이트_변환값_해석).G해석값_단순형(0)
			호출_인수, ok := 호출_인수_후보.(xt.I호출_인수)

			if !ok {
				에러 := lib.New에러with출력("'I호출_인수'형이 아님 : '%T', '%v'",
					호출_인수_후보, 수신_메시지.G자료형_문자열(0))
...
func f소켓_질의_회신(회신_메시지 lib.I소켓_메시지, raw메시지 *mangos.Message, ch종료 chan lib.T신호) {
	...
	에러체크(회신_메시지.S소켓_회신(소켓REP_TR수신, lib.P10초, raw메시지))
}
