...
func c함수_호출_도우미(ch초기화 chan<- lib.T신호) (에러 error) {
	...
	for {
		...
		default:
			...
			질의값, 에러 := lib.F소켓_메시지_해석with지정된_해석함수(수신_메시지, 0, xing.F바이트_변환값_해석)
			lib.F에러체크(에러)
			...
			switch 함수 {
			case xing.P함수_질의:
				질의값 := 질의값.(xing.S호출_인수_질의).M질의값
				식별번호, 에러 := f조회_및_주문_질의(질의값)
			...
			case xing.P함수_접속:
				...
				성공_여부 := F접속(서버_구분)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, 성공_여부)
			case xing.P함수_로그인:
				성공_여부 := F로그인()
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, 성공_여부)
			case xing.P함수_접속됨:
			...
			case xing.P함수_에러_메시지:
				s := 질의값.(xing.S호출_인수_정수값)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F에러_메시지(s.M정수값))
			case xing.P함수_TR쿼터:
				s := 질의값.(xing.S호출_인수_문자열)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F초당_TR쿼터(s.M문자열))
			case xing.P함수_계좌_수량:
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_수량())
			case xing.P함수_계좌_이름:
				s := 질의값.(xing.S호출_인수_문자열)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_이름(s.M문자열))
			case xing.P함수_계좌_상세명:
				s := 질의값.(xing.S호출_인수_문자열)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, F계좌_상세명(s.M문자열))
			case xing.P함수_압축_해제:
				s := 질의값.(xing.S호출_인수_압축_해제)
				복원값 := F압축_해제(unsafe.Pointer(&s.M바이트_모음), s.M길이)
				회신_메시지, 에러 = lib.New소켓_메시지(변환_형식, 복원값)
			}
...
func f조회_및_주문_질의(질의값 lib.I질의값) (식별번호 int, 에러 error) {
	defer lib.F에러패닉_처리(lib.S에러패닉_처리{
		M에러: &에러,
		M함수: func() { 식별번호 = 0 }})
