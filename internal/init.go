...
func init() {
	...
	lib.F메모("종목별 재무/실적 데이터 확보방안 마련할 것.")
	lib.F메모("선물 가격 정보 수집 기능 구현할 것. (t8415, t8416, t9943)")
	lib.F메모("업종 지수 정보 수집 기능 구현할 것. (t8419, t8424)");
...
func f초기화_TR전송_제한() {
	...
	for TR코드, 초당_제한_횟수 := range 코드별_10분당_TR전송_제한 {
		tr전송_코드별_10분당_제한[TR코드] = lib.New전송_권한(TR코드, 초당_제한_횟수, lib.P10분)
	}
	...
	for TR코드, 초당_제한_횟수 := range 코드별_초당_TR전송_제한 {
		tr전송_코드별_초당_제한[TR코드] = lib.New전송_권한(TR코드, 초당_제한_횟수, lib.P1초)
	}
}
