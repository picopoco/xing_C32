...
func New현물호가조회(tr *TR_DATA) (s *xt.S현물호가조회_응답, 에러 error) {
	...
	s = new(xt.S현물호가조회_응답)
	s.M한글명 = lib.F2문자열_CP949(g.Hname)
	...
	s.M수신시간 = lib.F2금일_시각_단순형("150405.99", 시각_문자열[:6]+"."+시각_문자열[6:])
	s.M예상체결가격 = lib.F2정수64_단순형(g.Yeprice)
	s.M예상체결수량 = lib.F2정수64_단순형(g.Yevolume)
	s.M예상체결전일구분 = xt.T전일대비_구분(lib.F2정수64_단순형(g.Yesign))
	...
	s.M동시호가_구분 = xt.T동시호가_구분(lib.F2정수64_단순형(g.Status))
...
func New현물시세조회_응답(tr *TR_DATA) (s *xt.S현물시세조회_응답, 에러 error) {
	defer lib.F에러패닉_처리(lib.S에러패닉_처리{
		M에러: &에러,
		M함수: func() {
			lib.F문자열_출력("'%v' : '%v'", s.M종목코드, s.M한글명)
			s = nil
		}})
	...
	s.M매도증권사명_모음[0] = lib.F2문자열_CP949(g.Offerno1)
	s.M매수증권사명_모음[0] = lib.F2문자열_CP949(g.Bidno1)
	...
	s.M매도증권사명_모음[1] = lib.F2문자열_CP949(g.Offerno2)
	s.M매수증권사명_모음[1] = lib.F2문자열_CP949(g.Bidno2)
	...
	s.M매도증권사명_모음[2] = lib.F2문자열_CP949(g.Offerno3)
	s.M매수증권사명_모음[2] = lib.F2문자열_CP949(g.Bidno3)
	...
	s.M매도증권사명_모음[3] = lib.F2문자열_CP949(g.Offerno4)
	s.M매수증권사명_모음[3] = lib.F2문자열_CP949(g.Bidno4)
	...
	s.M매도증권사명_모음[4] = lib.F2문자열_CP949(g.Offerno5)
	s.M매수증권사명_모음[4] = lib.F2문자열_CP949(g.Bidno5)
	...
	s.M전분기명 = lib.F2문자열_CP949_공백제거(g.Name)
	...
	s.M전전분기명 = lib.F2문자열_CP949_공백제거(g.Name2)
	...
	s.M락구분 = lib.F2문자열_CP949(g.Info1)
	s.M관리_급등구분 = lib.F2문자열_CP949(g.Info2)
	s.M정지_연장구분 = lib.F2문자열_CP949(g.Info3)
	s.M투자_불성실구분 = lib.F2문자열_CP949(g.Info4)
	...
	s.M투자주의환기 = lib.F2문자열_CP949_공백제거(g.Info5)
	s.M기업인수목적회사여부 = lib.F2참거짓(g.Spac_gubun, "N", false)
	s.M발행가격 = lib.F2정수64_단순형(g.Issueprice)
	s.M배분적용구분코드 = lib.F2문자열_CP949(g.Alloc_gubun)
	s.M배분적용구분 = lib.F2문자열_CP949(g.Alloc_text)
	s.M단기과열_VI발동 = lib.F2문자열_CP949(g.Shterm_text)
	...
	대차불가표시_문자열 := lib.F2문자열_CP949_공백제거(g.Lend_text)
	switch 대차불가표시_문자열 {
	case "":
		s.M대차불가여부 = false
	case "대차불가":
		s.M대차불가여부 = true
	default:
		lib.F패닉("%v '대차불가표시_문자열' 예상하지 못한 값 : '%v'", s.M종목코드, 대차불가표시_문자열)
	}
...
func New현물_정상주문_응답1(tr *TR_DATA) (s *xt.S현물_정상주문_응답1, 에러 error) {
	...
	s.M신용거래_구분 = f2신용거래_구분(xt.T신용거래_구분(lib.F2문자열_공백제거(g.MgntrnCode)))
	s.M대출일 = lib.F2포맷된_일자_단순형_공백은_초기값("20060102", g.LoanDt)
	s.M회원번호 = lib.F2문자열_공백제거(g.MbrNo)
	s.M주문조건_구분 = f2주문조건(xt.T주문조건(lib.F2문자열_공백제거(g.OrdCndiTpCode)))
...
func New현물_정상주문_응답2(tr *TR_DATA) (s *xt.S현물_정상주문_응답2, 에러 error) {
	...
	s.M주문시장_코드 = xt.T주문_시장구분(lib.F2정수_단순형(g.OrdMktCode))
...
func NewCSPAT00700InBlock(질의값 *xt.S질의값_정정주문) (s *CSPAT00700InBlock1) {
	...
	lib.F바이트_복사_문자열(s.IsuNo[:], 질의값.M종목코드)
	lib.F바이트_복사_정수(s.OrdQty[:], 질의값.M주문수량)
	lib.F바이트_복사_문자열(s.OrdprcPtnCode[:], string(f2Xing호가유형(질의값.M호가유형)))
	lib.F바이트_복사_문자열(s.OrdCndiTpCode[:], string(f2Xing주문조건(질의값.M주문조건)))
	lib.F바이트_복사_정수(s.OrdPrc[:], 질의값.M주문단가)
...
func New현물_정정주문_응답1(tr *TR_DATA) (s *xt.S현물_정정주문_응답1, 에러 error) {
	...
	s.M주문수량 = lib.F2정수64_단순형(g.OrdQty)
	s.M호가유형 = f2호가유형(xt.T호가유형(lib.F2문자열_공백제거(g.OrdprcPtnCode)))
	s.M주문조건 = f2주문조건(xt.T주문조건(lib.F2문자열_공백제거(g.OrdCndiTpCode)))
	s.M주문가격 = lib.F2정수64_단순형(g.OrdPrc)
	s.M통신매체_코드 = lib.F2문자열_공백제거(g.CommdaCode)
	s.M전략코드 = lib.F2문자열_공백제거(g.StrtgCode)
	s.M그룹ID = lib.F2문자열_공백제거(g.GrpId)
...
func New현물_정정주문_응답2(tr *TR_DATA) (s *xt.S현물_정정주문_응답2, 에러 error) {
	...
	s.M주문시장_코드 = xt.T주문_시장구분(lib.F2정수64_단순형_공백은_0(g.OrdMktCode))
	s.M주문유형_코드 = lib.F2문자열_공백제거(g.OrdPtnCode)
...
func New현물_취소주문_응답2(tr *TR_DATA) (s *xt.S현물_취소주문_응답2, 에러 error) {
	...
	s.M공매도_호가구분 = lib.F2문자열_공백제거(g.StslOrdprcTpCode)
	s.M공매도_가능 = lib.F문자열_비교(g.StslAbleYn, "Y", true)
	s.M신용거래_코드 = f2신용거래_구분(xt.T신용거래_구분(lib.F2문자열_공백제거(g.MgntrnCode)))
	s.M대출일 = lib.F2포맷된_일자_단순형_공백은_초기값("20060102", g.LoanDt)
	s.M반대매매주문_구분 = lib.F2문자열_공백제거(g.CvrgOrdTp)
...
func NewETF시간별_추이_응답_헤더(tr *TR_DATA) (s *xt.S_ETF시간별_추이_응답_헤더, 에러 error) {
	...
	s.M업종지수명 = lib.F2문자열_CP949_공백제거(g.UpName)
...
func NewETF시간별_추이_응답_반복값_모음(tr *TR_DATA) (반복값_모음 []*xt.S_ETF시간별_추이_응답_반복값, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		if lib.F2문자열_CP949(g.Time) == "장:마:감" {
			s.M시각 = f2당일_시각_단순형("15:04:05", g_모음[i+1].Time).Add(lib.P10초)
		} else {
			s.M시각 = f2당일_시각_단순형("15:04:05", g.Time)
		}

		s.M현재가 = lib.F2정수64_단순형(g.Price)
		s.M전일대비구분 = xt.T전일대비_구분(lib.F2정수_단순형(g.Sign))
...
func NewT8428InBlock(질의값 *xt.S질의값_증시주변자금추이) (g *T8428InBlock) {
	...
	lib.F조건부_패닉(질의값.TR코드 != xt.TR증시_주변_자금_추이, "예상하지 못한 TR코드 : '%v'", 질의값.TR코드)
...
func New증시주변자금추이_응답_반복값_모음(tr *TR_DATA) (반복값_모음 []*xt.S증시주변자금추이_응답_반복값, 에러 error) {
	...
	for _, g := range g_모음 {
		...
		if len(시각_문자열_원본) != 8 {
			//lib.F문자열_출력("예상과 다른 시각 문자열 길이 : '%v', '%v'", len(시각_문자열_원본), 시각_문자열_원본)
			return 반복값_모음, nil
		}
		...
		s.M지수 = lib.F2실수_단순형(g.Jisu)
		s.M전일대비_구분 = xt.T전일대비_구분(lib.F2정수64_단순형(g.Sign))
...
func NewT8436InBlock(질의값 *lib.S질의값_문자열) (g *T8436InBlock) {
	...
	lib.F조건부_패닉(질의값.TR구분 != lib.TR조회, "예상하지 못한 TR구분 : '%v'", 질의값.TR구분)
	lib.F조건부_패닉(질의값.TR코드 != xt.TR현물_종목_조회, "예상하지 못한 TR코드 : '%v'", 질의값.TR코드)
...
func New현물_주식종목조회_모음(g_모음 []T8436OutBlock) (값_모음 []*xt.S주식종목조회_응답, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		s.M기준가 = lib.F2정수64_단순형(g.RecPrice)
		s.M증권그룹 = xt.T증권그룹(lib.F2정수_단순형(g.Bu12Gubun))
		...
		default:
			panic(lib.F포맷된_문자열("예상하지 못한 경우 : '%v', '%v'", ETF구분, 시장구분))
		}

		값_모음[i] = s

		switch {
		case s.M증권그룹 == xt.P증권그룹_상장지수펀드_ETF && s.M시장구분 == lib.P시장구분_ETN,
			s.M증권그룹 == xt.P증권그룹_ETN && s.M시장구분 == lib.P시장구분_ETF:
			lib.F문자열_출력(
