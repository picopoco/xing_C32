...
func New현물시세조회_응답(tr *TR_DATA) (s *xt.S현물_시세조회_응답, 에러 error) {
	defer lib.S에러패닉_처리기{
		M에러_포인터: &에러,
		M함수: func() {
...
func New현물_기간별_조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S현물_기간별_조회_응답_반복값_모음, 에러 error) {
	...
	값.M배열 = make([]*xt.S현물_기간별_조회_응답_반복값, 배열_길이, 배열_길이)
...
func New현물_당일전일분틱조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S현물_전일당일분틱조회_응답_반복값_모음, 에러 error) {
	...
	lib.F조건부_패닉(당일전일_구분 != xt.P당일전일구분_전일 && 당일전일_구분 != xt.P당일전일구분_당일,
	...
	값.M배열 = make([]*xt.S현물_전일당일분틱조회_응답_반복값, 배열_길이, 배열_길이)
...
func New증시주변자금추이_응답_반복값_모음(tr *TR_DATA) (값 *xt.S증시주변자금추이_응답_반복값_모음, 에러 error) {
	...
	값.M배열 = make([]*xt.S증시주변자금추이_응답_반복값, 배열_길이, 배열_길이)
...
func New주식종목조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S현물_종목조회_응답_반복값_모음, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()
	...
	값 = new(xt.S현물_종목조회_응답_반복값_모음)
	값.M배열 = make([]*xt.S현물_종목조회_응답_반복값, 배열_길이, 배열_길이)

	for i, g := range g_모음 {
		s := new(xt.S현물_종목조회_응답_반복값)
		s.M종목명 = lib.F2문자열_EUC_KR_공백제거(g.HName)
		s.M종목코드 = lib.F2문자열_공백제거(g.ShCode)
