...
func NewETF시간별_추이_응답_반복값_모음(tr *TR_DATA) (값 *xt.S_ETF시간별_추이_응답_반복값_모음, 에러 error) {
	...
	값 = new(xt.S_ETF시간별_추이_응답_반복값_모음)
	값.M배열 = 배열

	return 값, nil
}

func NewT1305InBlock(질의값 *xt.S질의값_현물_기간별_조회) (g *T1305InBlock) {
	g = new(T1305InBlock)
	lib.F바이트_복사_문자열(g.Shcode[:], 질의값.M종목코드)
...
func New현물_기간별_조회_응답_헤더(tr *TR_DATA) (값 *xt.S현물_기간별_조회_응답_헤더, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()

	g := (*T1305OutBlock)(unsafe.Pointer(tr.Data))

	값 = new(xt.S현물_기간별_조회_응답_헤더)
...
func New현물_기간별_조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S현물_기간별_조회_응답_반복값_모음, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()
	...
	배열 := make([]*xt.S현물_기간별_조회_응답_반복값, len(g_모음), len(g_모음))
...
func New현물_당일전일분틱조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S현물_전일당일분틱조회_응답_반복값_모음, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()
	...
	값 = new(xt.S현물_전일당일분틱조회_응답_반복값_모음)
	값.M배열 = 배열
...
func New증시주변자금추이_응답_반복값_모음(tr *TR_DATA) (값 *xt.S증시주변자금추이_응답_반복값_모음, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()
...
func New주식종목조회_응답_반복값_모음(tr *TR_DATA) (값 *xt.S주식종목조회_응답_반복값_모음, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 값 = nil }}.S실행()
	...
	배열 := make([]*xt.S주식종목조회_응답_반복값, len(g_모음), len(g_모음))
