...
func New현물시세조회_응답(tr *TR_DATA) (s *xt.S현물시세조회_응답, 에러 error) {
	...
	case "대차불가":
		s.M대차불가여부 = true
	default:
		panic(lib.F2문자열("%v '대차불가표시_문자열' 예상하지 못한 값 : '%v'", s.M종목코드, 대차불가표시_문자열))
	}
...
func NewCSPAT00700InBlock(질의값 *xt.S질의값_정정주문) (g *CSPAT00700InBlock1) {
...
func NewT8428InBlock(질의값 *xt.S질의값_증시주변자금추이) (g *T8428InBlock) {
	...
	case lib.P시장구분_코스닥:
		시장구분_문자열 = "301"
	default:
		panic(lib.F2문자열("예상하지 못한 시장구분 값 : '%v'", 질의값.M시장구분))
	}
...
func New현물_주식종목조회_모음(g_모음 []T8436OutBlock) (값_모음 []*xt.S주식종목조회_응답, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		default:
			panic(lib.F2문자열("예상하지 못한 경우 : '%v', '%v'", ETF구분, 시장구분))
		}
