...
func f초기화_설정화일() {
	l.F조건부_패닉(l.F파일_없음(설정화일_경로), "설정화일 config.ini를 찾을 수 없습니다.")
}
...
func f초기화_XingAPI() {
	...
	l.F에러체크(os.Chdir(xing디렉토리))

	// XingAPI 초기화 ('반드시' DLL파일이 있는 디렉토리에서 실행해야 함.)
	C.initXingApi(0)

	// 원래 디렉토리로 이동
	l.F에러체크(os.Chdir(원래_디렉토리))
}

func f초기화_Go루틴() {
	ch초기화 := make(chan l.T신호)
...
func f초기화_소켓() {
	var 에러 error
	소켓PUB_콜백, 에러 = l.New소켓PUB(l.P주소_Xing_C함수_콜백)
	l.F에러체크(에러)

	소켓PUB_실시간_정보, 에러 = l.New소켓PUB(l.P주소_Xing_실시간)
	l.F에러체크(에러)
...
func f초기화_서버_접속() {
	소켓_질의 := l.New소켓_질의(l.P주소_Xing_C함수_호출, l.P변환형식_기본값, l.P30초)
	응답 := 소켓_질의.S질의(&(xt.S호출_인수_단순형{M함수: xt.P함수_접속})).G응답()

	if 응답.G에러() != nil && l.F테스트_모드_실행_중() {
		l.F체크포인트(응답.G에러())

		l.F문자열_출력("모의투자 기간이 종료되었는 지 확인하십시오.\n%v", 응답.G에러())
	}
...
func f초기화_TR전송_제한() {
	...
	for TR코드, 초당_제한_횟수 := range 코드별_10분당_TR전송_제한 {
		tr전송_코드별_10분당_제한[TR코드] = l.New전송_권한_TR코드별(TR코드, 초당_제한_횟수, l.P10분)
	}
	...
	for TR코드, 초당_제한_횟수 := range 코드별_초당_TR전송_제한 {
		tr전송_코드별_초당_제한[TR코드] = l.New전송_권한_TR코드별(TR코드, 초당_제한_횟수, l.P1초)
	}
}
...
func f초기화_영업일_기준_전일_당일() {
	...
	데이터, 에러 := l.New바이트_변환_매개체(l.P변환형식_기본값, 질의값)
	l.F에러체크(에러)

	호출_인수 := xt.New호출_인수_질의(데이터)

	응답 := l.New소켓_질의(l.P주소_Xing_C함수_호출, l.P변환형식_기본값, l.P30초).S질의(호출_인수).G응답()
	l.F에러체크(응답.G에러())

	응답값 := new(xt.S기간별_주가_조회_응답)
	응답.G값(0, 응답값)

	l.F조건부_패닉(응답값.M헤더.M수량 != 2, "예상하지 못한 수량. %v", 응답값.M헤더.M수량)
	l.F조건부_패닉(len(응답값.M반복값_모음) != 2, "예상하지 못한 수량. %v", len(응답값.M반복값_모음))
