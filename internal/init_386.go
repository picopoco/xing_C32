...
func f초기화_소켓() {
	...
	소켓PUB_실시간_정보, 에러 = lib.New소켓PUB(lib.P주소_Xing_실시간)
	lib.F에러체크(에러)
...
func f초기화_영업일_기준_전일_당일() {
	...
	호출_인수 := xt.New호출_인수_질의()
	호출_인수.M함수 = xt.P함수_질의

	데이터, 에러 := lib.New바이트_변환_매개체(lib.P변환형식_기본값, 질의값)
	lib.F에러체크(에러)

	호출_인수.M질의값 = 데이터

	응답 := lib.New소켓_질의(lib.P주소_Xing_C함수_호출, lib.P변환형식_기본값, lib.P30초).S질의(호출_인수).G응답()
	lib.F에러체크(응답.G에러())
